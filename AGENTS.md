...
7. `lavish-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
   ...
   Default no-timeout polls stream whitespace heartbeat bytes before the final JSON response and write waiting banners to stderr only when stderr is an interactive terminal (`shouldNarratePollWait`), so agent harnesses with piped, non-TTY stdio get no wait-tick noise in their merged capture; stdout is always reserved for the final JSON/TOON response, and `--timeout-ms` is a non-streaming test/debug escape hatch.
