...
- **Agent presence** - The browser shows when no agent is listening, keeps queued feedback and proven severe layout failures for the next successful `lavish-axi poll` send even across reloads, and only blocks human sends while the agent is working on delivered feedback; the agent's reply (`--agent-reply`) concludes that work and re-enables sends.
  In an interactive terminal the no-timeout poll writes an immediate stderr banner and periodic wait banners; when stderr is piped (as under agent harnesses) those banners are suppressed so the captured output carries no wait-tick noise. Stdout always stays reserved for the final response; if the poll is interrupted or times out, re-run it because queued feedback is never lost.
